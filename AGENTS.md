...
| 用語 | 指すもの | 例 |
|---|---|---|
| **セクション** | テーマで束ねた親ディレクトリ（`sections/<section>/`） | `02-auth/`, `03-injection/`, `04-performance/` |
| **レクチャー** | 1つのハンズオン単位＝1つの動くアプリ（`sections/<section>/<lecture>/`） | `03-injection/01-sqli/`, `04-performance/03-cache/` |
...
| 02-auth | 01-no-authentication | 管理画面（ユーザー管理） | User |
| 02-auth | 02-weak-authentication | ログイン | User |
| 02-auth | 03-broken-authorization | ユーザー詳細（プロフィール） | User |
| 03-injection | 01-sqli | ログイン | User |
| 03-injection | 02-xss | 記事へのコメント表示 | Article, Comment |
| 03-injection | 03-command-injection | 記事の PDF エクスポート | Article |
...
### 02-auth / 01-no-authentication（認証なし）
...
### 02-auth / 02-weak-authentication（不十分な認証）
...
### 02-auth / 03-broken-authorization（不十分な認可）
...
### 03-injection / 01-sqli（SQL インジェクション）
...
### 03-injection / 02-xss（XSS / Stored XSS）
...
### 03-injection / 03-command-injection（コマンドインジェクション）
