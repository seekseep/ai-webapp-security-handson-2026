...
ナレッジ共有ツールの **プロフィール詳細 `/users/:id`** で、ログインしてさえいれば他人のメールアドレスやパスワードハッシュまで覗けてしまうサンプルアプリです。脆弱性を体験してから、コードを修正するまでがこの章の課題です。
...
| ファイル | 問題 |
|---|---|
| `app/routes/users.js` | ログイン中なら他人の `:id` でも閲覧でき、必要のない `password` まで取得・表示している |
