...
記事一覧画面が **全件取得・全件描画** していて遅いサンプル。シードも 50000 件に増やしてある。
...
npm run database:seed   # 初期データ投入（50000 件、数秒〜十数秒程度かかります）
npm run dev             # 開発サーバー起動（ファイル変更で自動再起動）
```

ブラウザで http://localhost:3000 にアクセスしてください。

> **メモ**：シード件数を 50000 件に増やしているため、`npm run database:seed` の完了までに数秒〜十数秒程度かかります。実行が止まっているように見えても、最後に件数ログが出るまで待ってください。
...
| `scripts/seed.js` | 体感のために記事を 50000 件投入している（重さの前提） |
