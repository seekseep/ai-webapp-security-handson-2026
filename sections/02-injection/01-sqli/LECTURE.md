...
4. パスワードを **平文で保存している箇所** を特定し、ハッシュ化して保存・検証する形に修正する
...
### TODO 4: 平文パスワードをハッシュ化する

SQL インジェクションを直しても、DB に平文でパスワードが入っているのは別の問題です。DB が漏洩した瞬間に全ユーザーのパスワードが筒抜けになります。

[app/lib/password.js](./app/lib/password.js) に `hashPassword` / `verifyPassword` があらかじめ用意されています。これを使って次の修正をしてください。

1. **新規登録時**（`auth.js` の `app.post('/register', ...)`）：保存前に `hashPassword(password)` でハッシュ化
2. **ログイン時**（`auth.js` の `app.post('/login', ...)`）：DB の値と直接比較せず、`verifyPassword(入力, 保存値)` で比較
3. **シードデータ**（`scripts/seed.js`）：`hashPassword(...)` を経由してから INSERT
4. 修正後に `npm run database:seed` で DB を作り直し、ログインできることを確認

修正の方向性としては、ログインクエリは email だけで引いて、パスワード照合は **アプリケーション側で `verifyPassword` を呼ぶ** のがシンプルです。
...
ログインの SQL インジェクションを直し、平文パスワード問題も解消できたら、次の章（XSS）に進みましょう。
